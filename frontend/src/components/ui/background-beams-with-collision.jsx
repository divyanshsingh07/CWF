...
export const BackgroundBeamsWithCollision = ({
  ...
  return (
    <div
      ref={parentRef}
      className={cn(
        "h-full w-full absolute inset-0 bg-linear-to-b from-white to-neutral-100 dark:from-neutral-950 dark:to-neutral-800 flex items-center justify-center overflow-hidden",
        className
      )}
    >
...
const CollisionMechanism = React.forwardRef(({ parentRef, containerRef, beamOptions = {} }, ref) => {
  ...
  useEffect(() => {
    if (collision.detected && collision.coordinates) {
      setTimeout(() => {
        setCollision({ detected: false, coordinates: { x: null, y: null } });
...
const Explosion = ({ className, style, ...props }) => {
  ...
  return (
    <div {...props} className={cn("absolute z-50 h-2 w-2", className)} style={style}>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
